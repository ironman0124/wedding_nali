...
function renderExpenses() {
  ...
  // Sheet columns: D=Total(R), E=Paid(R), F=Balance(R)
  // In Code.gs: budget=col D (Total), actual=col E (Paid), diff=col F (Balance)
  ...
  // Filter out TOTAL row and empty rows, use correct sheet columns
  const expRows = state.expenses.filter(e => e.name && String(e.name).toUpperCase() !== 'TOTAL' && e.cat);
  document.getElementById('expense-tbody').innerHTML = expRows.map((e,i) => {
    // budget=Total(R), actual=Paid(R), diff=Balance(R) from sheet
